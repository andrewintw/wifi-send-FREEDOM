...
原理就跟你使用手機分享熱點是一樣的，只是通常手機開啟熱點後，只能廣播一個 SSID。技術上也可以透過 MAC 地址溯源是哪個手機發出來的（一般來說，每個網路裝置都會有一個唯一的 MAC 地址，該資訊是設備生產時燒寫在硬體的）。但這個 WiFi SSID 廣播裝置會可同時發送多個 SSID 名稱，並且透過隨機產生 MAC 地址的方式隱蔽發送者的資訊。

從 libreddit 貼文下方的幾則討論中，可以抓幾個技術上的重點：
